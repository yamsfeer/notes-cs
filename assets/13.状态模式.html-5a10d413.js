import{_ as p}from"./plugin-vue_export-helper-c27b6911.js";import{r as o,o as c,c as e,a as s,b as n,d as u,e as a}from"./app-706bfe03.js";const l={},i=a(`<h1 id="状态模式" tabindex="-1"><a class="header-anchor" href="#状态模式" aria-hidden="true">#</a> 状态模式</h1><p>状态模式是一种非同寻常的优秀模式，它也许是解决某些需求场景的最好方法。</p><p>下面通过两个例子介绍状态模式。</p><h2 id="电灯程序" tabindex="-1"><a class="header-anchor" href="#电灯程序" aria-hidden="true">#</a> 电灯程序</h2><p>现有一个开关按钮，如果当前灯是关闭状态，点击按钮开灯，否则关灯，用代码表示如下：</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">let</span> state <span class="token operator">=</span> <span class="token string">&#39;off&#39;</span>
...
btn<span class="token punctuation">.</span>onclick <span class="token operator">=</span> buttonPress
</code></pre></div><p>如果某天又增加了强弱光，代码修改如下：</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token function">buttonPress</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  <span class="token keyword">if</span> <span class="token punctuation">(</span>state <span class="token operator">===</span> <span class="token string">&#39;off&#39;</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><p>以上代码有如下几个缺点：</p><ul><li>每次修改 light 的状态都需要改动 buttonPress 里的代码，违反开放-封闭原则。</li><li>逻辑都堆在 buttonPress 里，如果新增强强光、超强光，这个函数会继续膨胀。</li><li>状态转换不明显。在这一串 if-else 语句里，很难一眼看出到底有多少种状态以及他们的转换关系。</li></ul><h3 id="状态模式改进" tabindex="-1"><a class="header-anchor" href="#状态模式改进" aria-hidden="true">#</a> 状态模式改进</h3><p>状态模式的关键是：把状态和对应的行为独立封装起来，当 button 按下的时，只需在上下文中把请求委托给当前的状态对象即可。</p><p>这个例子中，灯有关闭、弱光、强光灯几个状态。</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">const</span> offState <span class="token operator">=</span> <span class="token punctuation">{</span>
...
<span class="token keyword">const</span> strongState <span class="token operator">=</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><p>每次点击按钮，需要改变当前状态，这会自动替换掉 buttonPress 函数。</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">Light</span> <span class="token punctuation">{</span>
...
button<span class="token punctuation">.</span><span class="token function-variable function">onclick</span> <span class="token operator">=</span> <span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token operator">=&gt;</span> light<span class="token punctuation">.</span>curState<span class="token punctuation">.</span><span class="token function">buttonPress</span><span class="token punctuation">(</span>light<span class="token punctuation">)</span>
</code></pre></div><p>每个 state 对象的 buttonPress 函数即是这个状态对应的行为。</p><h2 id="文件上传" tabindex="-1"><a class="header-anchor" href="#文件上传" aria-hidden="true">#</a> 文件上传</h2><p>不论是文件上传，还是音乐、视频播放器，都有明显的状态区分。</p><ul><li>文件上传中有扫描、正在上传、暂停、上传成功、上传失败等状态</li><li>音乐播放器有加载中、正在播放、暂停、播放完毕等状态。</li></ul><p>以文件上传为例，假设上传组件有两个按钮，一个用于暂停或继续上传，另一个用于删除文件。</p><ul><li><p>文件扫描状态时，不能暂停或删除</p><ul><li>如果文件已存在服务器，则直接跳到上传完成状态</li><li>如果文件损坏，则跳到失败状态</li><li>其余情况进入上传中状态</li></ul></li><li><p>上传中状态下可以暂停上传，已暂停状态可以继续上传</p></li><li><p>扫描和上传中状态下，不能删除文件，暂停、上传完成、上传失败的情况下才能删除</p></li></ul><p>和上个例子一样，先不用状态模式实现：</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">Upload</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><p>不难看出这段代码的缺点，太多 if-else 语句，状态和行为被耦合在一个巨大的方法里，难以拓展和维护。</p><h3 id="状态模式重构" tabindex="-1"><a class="header-anchor" href="#状态模式重构" aria-hidden="true">#</a> 状态模式重构</h3><p>状态模式在文件上传的程序中，是最优雅的解决办法之一。先来封装状态。</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">const</span> signState <span class="token operator">=</span> <span class="token punctuation">{</span>
...
<span class="token keyword">const</span> errorState <span class="token operator">=</span> <span class="token punctuation">{</span>
  ...
  <span class="token function">del</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span> upload<span class="token punctuation">.</span><span class="token function">del</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">Upload</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><h2 id="状态模式的几点" tabindex="-1"><a class="header-anchor" href="#状态模式的几点" aria-hidden="true">#</a> 状态模式的几点</h2><h3 id="优缺点" tabindex="-1"><a class="header-anchor" href="#优缺点" aria-hidden="true">#</a> 优缺点</h3><p>优点：</p><ul><li>通过增加新的状态类，很容易增加新的状态和转换关系。</li><li>避免 context 无限膨胀，状态切换的逻辑被分布在状态类中。</li><li>用对象代替字符串来记录状态，使得状态的切换一目了然。</li><li>context 中的请求动作和状态的行为互不影响。</li></ul><p>缺点：会在系统中定义许多状态类，增加不少对象。</p><h3 id="性能优化" tabindex="-1"><a class="header-anchor" href="#性能优化" aria-hidden="true">#</a> 性能优化</h3><p>我们有两种选择来管理 state 对象的创建和销毁。</p><ul><li>当 state 对象被需要时才创建并随后销毁</li><li>一开始就创建好所有状态对象，且始终不销毁</li></ul><p>如果 state 对象比较大，可以用第一种方式；如果状态改变很频繁，则使用第二种。</p><p>另外，各 context 对象可以共享一个 state 对象，这也是享元模式的应用场景之一。</p><h3 id="与策略模式的关系" tabindex="-1"><a class="header-anchor" href="#与策略模式的关系" aria-hidden="true">#</a> 与策略模式的关系</h3><p>策略模式和状态模式有点类似，它们都有上下文 context、策略或状态，上下文把请求委托给策略或状态来执行。他们的区别在于：</p><ul><li>策略类之间是平行的，它们之间没有任何联系， 客户必须熟知策略的具体作用，按需选择。</li><li>状态的行为、状态之间的切换是被封装好的，“改变行为”发生在状态模式内部，客户不需要了解细节。</li></ul><h2 id="总结" tabindex="-1"><a class="header-anchor" href="#总结" aria-hidden="true">#</a> 总结</h2><p>通过电灯和文件上传这两个例子不难看出，当程序状态较多时，很容易写出 if-else 的屎山代码。</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">let</span> state <span class="token operator">=</span> <span class="token number">1</span>
...
<span class="token punctuation">}</span>
<span class="token comment">// ...</span>
</code></pre></div><p>状态模式封装了状态与对应的行为，context 将请求委托给状态对象的行为。</p><p>状态模式的大概结构如下：</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">const</span> <span class="token constant">FSM</span> <span class="token operator">=</span> <span class="token punctuation">{</span>
  ...
  <span class="token comment">// ...</span>
<span class="token punctuation">}</span>
</code></pre></div><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">const</span> context <span class="token operator">=</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div>`,25);function d(g,h){const t=o("ExternalLinkIcon");return c(),e("div",null,[i,s("p",null,[n("github 上的状态机库："),s("a",k,[n("javascript-state-machine"),u(t)]),n("。")]),r])}const y=p(l,[["render",d],["__file","13.状态模式.html.vue"]]);export{y as default};
