import{_ as t}from"./plugin-vue_export-helper-c27b6911.js";import{r as o,o as e,c,a as n,b as s,d as p,e as l}from"./app-706bfe03.js";const r={},u=l(`<h1 id="flex、bison" tabindex="-1"><a class="header-anchor" href="#flex、bison" aria-hidden="true">#</a> flex、bison</h1><p>早期 Unix 的 lex / YACC，发展为 flex / bison，新版本的程序是向上兼容的，现常用 flex 和 bison。</p><p>flex 是词法分析器生成器，词法分析器可以将文本转化为一个个 token，这些 tokens 可以用 bison 处理。</p><h2 id="flex" tabindex="-1"><a class="header-anchor" href="#flex" aria-hidden="true">#</a> flex</h2><p>安装 flex 命令：</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>$ brew <span class="token function">install</span> flex
...
$
</code></pre></div><h3 id="一个-flex-程序" tabindex="-1"><a class="header-anchor" href="#一个-flex-程序" aria-hidden="true">#</a> 一个 flex 程序</h3><p>flex 的输入是一个 <code>.l</code> (.lex) 文件，下面通过一个 flex 程序实现类似 Unix 系统中的 <code>wc</code> 命令。</p><div class="language-c" data-ext="c"><pre class="language-c"><code><span class="token comment">// wc.l</span>
...
<span class="token punctuation">}</span>
</code></pre></div><p>具体的 flex 文件内容会在后面介绍。执行以下命令：</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>$ flex wc.l
...
$
</code></pre></div><p><code>flex wc.l</code> 命令可以得到 flex 生成的词法分析器 <code>lex.yy.c</code>，这是一个 C 程序。使用 gcc 编译器编译执行 <code>lex.yy.c</code>。</p><h3 id="flex-文件结构" tabindex="-1"><a class="header-anchor" href="#flex-文件结构" aria-hidden="true">#</a> flex 文件结构</h3><p>flex 文件的内容大部分是 C 语言，它主要包含三部分：</p><div class="language-C" data-ext="C"><pre class="language-C"><code>%{
...
/* C code */
</code></pre></div><p>第一段主要写一些变量、函数声明，或者 <code>#include</code> 一些文件；</p><p>第二段由多条 pattern 和对应的 action 组成，其中 pattern 是正则表达式，action 是一段 C 代码，当解析到对应 pattern 的token 就会执行对应的 action；</p><p>第三段也是 C 代码，它会被直接复制到生成的分析器代码 <code>lex.yy.c</code> 中。</p><h2 id="bison" tabindex="-1"><a class="header-anchor" href="#bison" aria-hidden="true">#</a> bison</h2><p>安装 bison：</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>$ brew <span class="token function">install</span> bison
...
$
</code></pre></div><h3 id="bison-文件结构" tabindex="-1"><a class="header-anchor" href="#bison-文件结构" aria-hidden="true">#</a> bison 文件结构</h3><p>bison 的文件结构和 flex 文件一样有三个部分，分别是 声明、规则和 C 语言部分，这不是巧合。</p><p>其中声明部分的代码会被复制到生成的语法分析器的代码开头。</p><p>和 flex 文件不同的是，bison 文件里有 %token 声明，用来告诉 bison 词法分析生成的 token 符号，</p><div class="language-c" data-ext="c"><pre class="language-c"><code><span class="token operator">%</span><span class="token punctuation">{</span>
...
<span class="token comment">/* C code */</span>
</code></pre></div><p>第二部分主要是简化的 BNF（形式更像是 EBNF）以及产生式对应的语义动作，语义动作是 C 语言编写。</p><p>在产生式的语义动作中，冒号左边的用 <code>$$</code> 表示，其余用 <code>$1</code>、<code>$2</code>等符号表示。</p><div class="language-c" data-ext="c"><pre class="language-c"><code>exp<span class="token operator">:</span> factor       <span class="token comment">/* default $$ = $1 */</span>
 ...
 <span class="token punctuation">;</span>
</code></pre></div><h2 id="结合-flex-和-bison" tabindex="-1"><a class="header-anchor" href="#结合-flex-和-bison" aria-hidden="true">#</a> 结合 flex 和 bison</h2><p>flex 和 bison 的协作关系：</p><img src="https://raw.githubusercontent.com/yamsfeer/pic-bed/master/008i3skNgy1gxozrxriqij30ib0avwez.jpg" style="zoom:80%;"><p>以四则运算为例，用 flex 和 bison 写一个运算器。</p><p>定义词法：</p><div class="language-c" data-ext="c"><pre class="language-c"><code><span class="token comment">/* calc.l */</span>
<span class="token operator">%</span><span class="token punctuation">{</span>
...
<span class="token operator">%</span><span class="token operator">%</span>
</code></pre></div><p>定义语法：</p><div class="language-c" data-ext="c"><pre class="language-c"><code><span class="token comment">/* calc.y */</span>
<span class="token operator">%</span><span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre></div><p>写个 makefile 方便测试：</p><div class="language-makefile" data-ext="makefile"><pre class="language-makefile"><code><span class="token target symbol">all</span><span class="token punctuation">:</span> clean calc
...
</code></pre></div><p>执行：</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>$ <span class="token function">make</span>
$ ./calc
...
Mystery character a
error: syntax error
</code></pre></div><h2 id="参考" tabindex="-1"><a class="header-anchor" href="#参考" aria-hidden="true">#</a> 参考</h2>`,42),k={href:"https://www.oreilly.com/library/view/flex-bison/9780596805418/ch01.html",target:"_blank",rel:"noopener noreferrer"},i={href:"https://zhuanlan.zhihu.com/p/120812270",target:"_blank",rel:"noopener noreferrer"};function d(h,f){const a=o("ExternalLinkIcon");return e(),c("div",null,[u,n("p",null,[n("a",k,[s("flex & bison by John Levine"),p(a)])]),n("p",null,[n("a",i,[s("Flex(scanner)/Bison(parser)工作原理"),p(a)])])])}const x=t(r,[["render",d],["__file","附录B. flex、bison.html.vue"]]);export{x as default};
