import{_ as n}from"./plugin-vue_export-helper-c27b6911.js";import{o as s,c as a,e as p}from"./app-706bfe03.js";const t={},o=p(`<h1 id="堆-heap" tabindex="-1"><a class="header-anchor" href="#堆-heap" aria-hidden="true">#</a> 堆 ( heap )</h1><p>堆可分为最大堆，最小堆。</p><ul><li>最大堆：所有子树的根节点不小于左右孩子</li><li>最小堆：所有子树的根节点不大于左右孩子</li></ul><p>堆和二叉搜索树的区别在于：堆的每个节点作为根节点与左右孩子比较，二叉搜索树则是每个节点作为根节点与左右子树的比较。</p><p>最大最小堆都是完全二叉树，因此可用数组来存储堆。</p><div class="language-text" data-ext="text"><pre class="language-text"><code>      0
    /   \\
   1     2
  / \\   / \\
 3   4  5
</code></pre></div><p>在数组中的存储刚好是 012345，且有:</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token function">parent</span><span class="token punctuation">(</span>i<span class="token punctuation">)</span> <span class="token operator">=</span> <span class="token punctuation">(</span>i <span class="token operator">-</span> <span class="token number">1</span><span class="token punctuation">)</span> <span class="token operator">/</span> <span class="token number">2</span>
...
<span class="token function">rchild</span><span class="token punctuation">(</span>i<span class="token punctuation">)</span> <span class="token operator">=</span> i <span class="token operator">*</span> <span class="token number">2</span> <span class="token operator">+</span> <span class="token number">2</span>
</code></pre></div><h2 id="实现" tabindex="-1"><a class="header-anchor" href="#实现" aria-hidden="true">#</a> 实现</h2><p>我们用一个数组存储堆节点，从 0 索引开始存储。</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">MaxHeap</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><p>这里有两个重要操作 sift up 和 sift down，用于维护堆和合法性。</p><h3 id="siftup" tabindex="-1"><a class="header-anchor" href="#siftup" aria-hidden="true">#</a> siftUp</h3><p>以最大堆为例，当将一个新元素 E 插入数组末尾时，可能会破坏堆序性，但这个新元素只会影响它所在的子树，</p><ul><li><p>如果 E &gt; parent(E)，则与父节点交换</p></li><li><p>上一步交换后可能仍不满足堆序性，则需要逐层向上交换直到根节点，最终再次满足堆特性。</p></li></ul><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">Heap</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><h3 id="siftdown" tabindex="-1"><a class="header-anchor" href="#siftdown" aria-hidden="true">#</a> siftDown</h3><p>以最大堆为例，在删除最大元delMax操作中，</p><ul><li><p>将数组最后一个元素 E 和堆顶元素 Max 交换，删除 Max</p></li><li><p>将交换后的堆顶元素 E 与它的左右孩子比较，并将 E 与左右孩子中的较大者交换，循环直至 E 为叶子节点</p></li></ul><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">Heap</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><h3 id="heapify" tabindex="-1"><a class="header-anchor" href="#heapify" aria-hidden="true">#</a> heapify</h3><p>heapify 就是将一个已存在的数组转化成堆。</p><p>简单做法是从左到右循环，针对每个节点做一次 add 操作，时间复杂度是 O(nlogn)。</p><p>事实上，我们可以将所有叶子节点都视为已经符合堆性质，然后从第一个非叶子结点开始，从后往前直到根节点做 sift down 操作即可。其中，第一个非叶子节点就是最后一个节点的父节点。</p><p>这称为 Floyd 算法，时间复杂度为 O(n)，复杂度的计算过程比较复杂，这里不介绍。</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">Heap</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><h3 id="replace" tabindex="-1"><a class="header-anchor" href="#replace" aria-hidden="true">#</a> replace</h3><p>replace 操作就是取出当前堆顶元素，然后替换成另一个元素 E，并且要保持堆性质。</p><p>简单的做法是先 pop 操作，然后 add(E)，这样需要两次 logn 的时间复杂度。</p><p>我们可以直接将堆顶元素替换成 E，然后进行 sift down，这样只需一次 logn 时间复杂度的操作。</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">Heap</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><h2 id="优先队列-priority-queue" tabindex="-1"><a class="header-anchor" href="#优先队列-priority-queue" aria-hidden="true">#</a> 优先队列 ( priority queue )</h2><p>用堆来实现优先队列是非常容易的，对于最大堆来说，节点的值越大，其优先级越高，同理，最小堆节点的值越小，其优先级越高。</p><p>对于怎样才算优先级高，可以用一个 compare 函数表达，我们要改动的是将 sift down 和 sift up 过程中关于比较节点大小的部分用 compare 函数替代。</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">PriorityQueue</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><p>当使用这个优先级队列时，需要传入定义优先级高的函数：</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">const</span> pq <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">PriorityQueue</span><span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token parameter">a<span class="token punctuation">,</span> b</span><span class="token punctuation">)</span> <span class="token operator">=&gt;</span> a <span class="token operator">-</span> b<span class="token punctuation">)</span> <span class="token comment">// 值越大优先级越高，等同于最大堆</span>
</code></pre></div><h3 id="优先队列的应用" tabindex="-1"><a class="header-anchor" href="#优先队列的应用" aria-hidden="true">#</a> 优先队列的应用</h3><p>优先队列的一个经典问题是：在 n 个元素中选出前 m 个元素。比如，在 100,000 个数字中选出前 100 个数字。</p><p>如果使用排序，时间复杂度为 nlogn，如果使用优先队列，时间复杂度为 nlogm。</p><p>我们可以用一个最小堆维护前 m 个节点，其余元素依次与堆顶元素比较，如果堆顶元素更大，则进行 replace。</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token function">headM</span><span class="token punctuation">(</span><span class="token parameter">arr</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre></div><h3 id="leetcode-347-题-前-k-个高频元素" tabindex="-1"><a class="header-anchor" href="#leetcode-347-题-前-k-个高频元素" aria-hidden="true">#</a> leetcode 347 题：前 K 个高频元素</h3><p>给你一个整数数组 nums 和一个整数 k ，请你返回其中出现频率前 k 高的元素。你可以按任意顺序返回答案。</p><p>这就是上面提到的“在 m 个元素中选出前 m 个元素”的题目，只是我们需要先统计一次 nums 中元素的频次。</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token function">topKFrequent</span><span class="token punctuation">(</span><span class="token parameter">nums<span class="token punctuation">,</span> k</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre></div><p>首先，在这个问题中，频次高表示优先级高，但是我们使用的是最小堆，因为我们需要堆元素中，频次最低的在堆顶，后续节点只需频次高于堆顶元素即可入堆。</p><h2 id="堆排序-heap-sort" tabindex="-1"><a class="header-anchor" href="#堆排序-heap-sort" aria-hidden="true">#</a> 堆排序 ( heap sort )</h2><p>使用堆进行排序，只需要先对目标序列进行 heapify 操作，使其成为最大堆或最小堆，然后依次取出堆顶元素直至堆为空。</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token function">heapSort</span><span class="token punctuation">(</span><span class="token parameter">arr</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre></div><p>前面提到过，heapify 操作的复杂度是 O(n)，n 次取出堆顶元素的时间复杂度是 O(nlogn)。</p>`,51),e=[o];function c(u,l){return s(),a("div",null,e)}const r=n(t,[["render",c],["__file","堆.html.vue"]]);export{r as default};
