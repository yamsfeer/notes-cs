import{_ as n}from"./plugin-vue_export-helper-c27b6911.js";import{o as s,c as a,e as t}from"./app-706bfe03.js";const p={},o=t(`<h1 id="滑动窗口" tabindex="-1"><a class="header-anchor" href="#滑动窗口" aria-hidden="true">#</a> 滑动窗口</h1><p>滑动窗口属于快慢指针的一种，两个指针之间的部分就是窗口，算法通过扩大或缩小窗口来解决问题。</p><p>滑动窗口的整体代码如下：</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">let</span> left <span class="token operator">=</span> <span class="token number">0</span><span class="token punctuation">,</span> right <span class="token operator">=</span> <span class="token number">0</span>
...
<span class="token punctuation">}</span>
</code></pre></div><p>滑动窗口通常用于处理<strong>字符串子串</strong>的问题，滑动窗口的整体思路不难，难的是各种细节，比如如何向窗口中添加新元素，如何缩小窗口，在窗口滑动的哪个阶段更新结果等。</p><p>滑动窗口算法的代码框架：</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token function">slideWindow</span><span class="token punctuation">(</span><span class="token parameter">s</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre></div><p>虽然有一个嵌套的 while 循环，但算法的时间复杂度仍是 O(n)。因为字符串中的每个元素都<strong>只会进入窗口一次</strong>，然后<strong>被移出窗口一次</strong>。</p><h2 id="最小覆盖子串-76" tabindex="-1"><a class="header-anchor" href="#最小覆盖子串-76" aria-hidden="true">#</a> 最小覆盖子串 ( 76 )</h2><p>在一个字符串 source 中找出包含 target 字符串中所有字符的最短子串。</p><p>整个算法的思路如下：</p><ol><li>定义一个左开右闭区间 [left, right) 作为窗口</li><li>不断增大 right 扩大窗口，直到窗口中的子串包含 target 所有字符</li><li>停止增加 right，转而增加 left 使窗口缩小，直到窗口子串不符合要求，即不再包含 target 所有字符</li><li>重复 2、3 步，直到 right 到达 source 的尾部</li></ol><p>第 2 步相当于寻找一个可行解，第 3 步优化这个可行解，最终找到最优解。</p><p>左右指针轮流前进，窗口大小增增减减，窗口不断向右滑动，这就是滑动窗口。</p><div class="hint-container tip"><p class="hint-container-title">Tips</p><p>理论上窗口区间可以用 [left, right] 或 (left, right)</p><ul><li>[left, right)：初始化时 [0, 0) 中没有元素，right 向右移动一位后 [0, 1) 包含一个元素</li><li>(left, right)：right 向右移动一位后 (0, 1) 仍然没有元素</li><li>[left, right]：初始区间 [0, 0] 就包含了一个元素</li></ul><p>[left, right) 是最方便的，其余两种情况都会给边界处理带来不必要的麻烦。</p></div><p>最小覆盖子串的最终代码：</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token function">minWindow</span><span class="token punctuation">(</span><span class="token parameter">src<span class="token punctuation">,</span> target</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre></div><ul><li>当我们发现 window 中的某个字符的数量等于 need 的所需数量，说明有一个字符覆盖完成了。</li><li>当所有字符都满足 need 中的数量，即 <code>count === need.size</code>，说明我们已经得到一个可行的子串，但还不是最小的，此时需要缩小窗口</li><li>缩小窗口前，要判断当前可行解是否更优</li><li>缩小窗口并记录子串长度，直到不满足 <code>count === need.size</code>，然后继续扩大窗口</li></ul><h2 id="字符串排列-567" tabindex="-1"><a class="header-anchor" href="#字符串排列-567" aria-hidden="true">#</a> 字符串排列 ( 567 )</h2><p>判断字符串 source 中是否包含由 target 的字符排列而成的子串。</p><p>相当给你一个 source 和一个 target，求 source 中是否存在一个子串，包含 target 中所有字符且不包含其他字符。</p><p>这个问题和上面的“最小覆盖子串”问题相比，可行解的限制条件更严格了，这个问题也是用滑动窗口算法。</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token function">checkInclusion</span><span class="token punctuation">(</span><span class="token parameter">source<span class="token punctuation">,</span> target</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre></div><p>这个问题基本和最小覆盖子串一样，只是子串中不能有 target 以外的字符。因此</p><ul><li>缩小窗口的时机是窗口大小等于 size 时，因为子串是由 target 排列而来</li><li>当 <code>count == need.size</code> 时，说明窗口中就是一个合法的排列，可以返回</li></ul><h2 id="找出所有字母异位词-438" tabindex="-1"><a class="header-anchor" href="#找出所有字母异位词-438" aria-hidden="true">#</a> 找出所有字母异位词 ( 438 )</h2><p>所谓字母异位词，就是字符串排列，这和上面的字符串排列是一样的，只是返回结果有点变化。</p><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token function">findAnagrams</span><span class="token punctuation">(</span><span class="token parameter">src<span class="token punctuation">,</span> target</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre></div><h2 id="最长无重复子串-3" tabindex="-1"><a class="header-anchor" href="#最长无重复子串-3" aria-hidden="true">#</a> 最长无重复子串 ( 3 )</h2><p>找出一个字符串中的最长无重复子串。</p><p>这道题比前面要简单，有两点需要注意：</p><ul><li>当 <code>window.get(c) &gt; 1</code> 说明窗口中存在重复字符，此时应该缩小窗口。</li><li>当缩小窗口完成后，窗口中是没有重复字符的，此时应该更新长度记录</li></ul><div class="language-javascript" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token function">lengthOfLongestSubstring</span><span class="token punctuation">(</span><span class="token parameter">s</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre></div><h2 id="总结" tabindex="-1"><a class="header-anchor" href="#总结" aria-hidden="true">#</a> 总结</h2><p>滑动窗口通常解决这样的问题：给定字符串 source，求 source 中一段满足特定条件的子串。</p><p>使用滑动窗口有三个问题要思考：</p><ul><li>什么时候扩大窗口？字符加入窗口时，应更新哪些数据？</li><li>什么时候缩小窗口？字符移出窗口时，应更新哪些数据？</li><li>什么时候更新可行解？扩大窗口时还是缩小窗口时？</li></ul>`,37),c=[o];function e(u,l){return s(),a("div",null,c)}const r=n(p,[["render",e],["__file","1.1.滑动窗口.html.vue"]]);export{r as default};
